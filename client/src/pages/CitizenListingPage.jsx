...
// import { getAllMissingAndCriminalsForUser } from '../apicalls/citizenapi';
...
// const CitizenListingPage = () => {
//     const user = useSelector(state => state.user.user);
//     const ispoliceDetails = useSelector(state => state.user.policeDetails);
...
//         queryKey: ['allMissingAndCriminals', user?.user_id,pincode],
//         queryFn: () => getAllMissingAndCriminalsForUser(pincode),
//         onSuccess: (data) => {
...
//     useEffect(() => {
//         applyFilters();
//     }, [dataType, criminalFilterType, criminalFilterValue, missingFilterType, missingFilterValue, criminalData, missingData,pincode]);
...
//     if (!pin) {
//       toast.error("Error fetching location. Make sure location permission is allowed.");
//     } else {
//       setPincode(pin);
//       toast.success(`Location fetched: ${pin}`);
//       queryClient.invalidateQueries(['allMissingAndCriminals', user?.user_id,pincode]);
//     }
//   } catch (error) {
//     console.error("Error getting location:", error);
//     toast.error("An unexpected error occurred while fetching location.");
//   } finally {
//     setLoading(false);
//   }
// };
// const handleResidentAreaListing = async () => {
//   setLoading(true);
//   try {
//     setPincode(null); // ensure pincode is null for resident area
//     await queryClient.invalidateQueries(['allMissingAndCriminals', user?.user_id, null]);
//   } catch (error) {
//     console.error("Error fetching resident area listings:", error);
//     toast.error("Failed to fetch listings for your resident area.");
//   } finally {
//     setLoading(false);
//   }
// };
...
//     if(loading) return <LoadingPage status={'load'} message={'getting your current location , please with ...'} />
//     return (
...
//     {/* Show dropdown and filters */}
//     <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 flex-grow">
...
//           className="block w-full sm:w-auto p-1.5 sm:p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-sm sm:text-base"
//         >
...
//         <>
//           <div className="flex items-center space-x-2 sm:space-x-3">
//             <label htmlFor="criminalFilterType" className="font-semibold text-gray-700 text-sm sm:text-base">Filter By:</label>
//             <select
...
//         <>
//           <div className="flex items-center space-x-2 sm:space-x-3">
//             <label htmlFor="missingFilterType" className="font-semibold text-gray-700 text-sm sm:text-base">Filter By:</label>
//             <select
...
//     {/* Search in current location button */}
//     <div className="flex-shrink-0">
//   {pincode ? (
//     <button
//       onClick={handleResidentAreaListing}
//       className="w-full md:w-auto bg-green-600 hover:bg-green-700 text-white font-semibold rounded-md px-4 py-2 text-sm sm:text-base shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors duration-200 ease-in-out"
//     >
//       See listings in your resident area
//     </button>
//   ) : (
//     <button
//       onClick={handleSearchCurrentLocation}
//       className="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-md px-4 py-2 text-sm sm:text-base shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-200 ease-in-out"
//     >
//       See listings in your current area
//     </button>
//   )}
// </div>
...
//                                         {!ispoliceDetails ? (
//                                             <>
//                                                 <svg className="mx-auto h-12 w-12 sm:h-16 sm:w-16 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
//                                                     <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M5 13l4 4L19 7" />
//                                                 </svg>
//                                                 <h3 className="mt-4 text-lg sm:text-xl font-medium text-green-700">No Listing in requested area or no filtered data</h3>
//                                                 <p className="mt-2 text-gray-600 text-sm sm:text-base">
//                                                     No crime or missing reports nearby your current location or locality. Stay alert and stay safe.
//                                                 </p>
//                                             </>
//                                         ) : (
//                                             <>
//                                                 <svg className="mx-auto h-12 w-12 sm:h-16 sm:w-16 text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
//                                                     <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 8c-1.657 0-3 1.343-3 3h6c0-1.657-1.343-3-3-3zm0 10c2.485 0 4.5-1.567 4.5-3.5h-9c0 1.933 2.015 3.5 4.5 3.5z" />
//                                                 </svg>
//                                                 <h3 className="mt-4 text-lg sm:text-xl font-medium text-blue-700">No listings in this location</h3>
//                                                 <p className="mt-2 text-gray-600 text-sm sm:text-base">
//                                                     Relax officers, no reports found in this area.
//                                                 </p>
//                                             </>
//                                         )}
//                                     </div>
...
//             {ispoliceDetails && !addCriminal && !addMissing && (
